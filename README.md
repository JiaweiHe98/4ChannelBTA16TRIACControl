...
The box can be compatible with fans range from 40mm to 70mm. It prevents electric shock caused by human contact and effectively dissipate the heat generated by the controller. For each of the base piece and the cover piece, a perfectly plain surface is designed to make the 3D printing process easier. The CAD file in both ```.SLDPRT``` and ```.STL``` can be found in the Dimmer Box CAD directory of this project.

### Dimmer
<img src="./img/dimmer.jpg">
The control circuit is powered by outside DC sources. The main AC will only power the loads. We need to use jumper cables to connect the dimmer with the Arduino. 7 cables are required; one for VCC, one for GND, one for Zero-Crossing signal, and four for control pulse.

### Testing
<img src="./img/Testing.gif">
Since we only have one LED at the testing time, we connected the only LED we have to each channel and do the testing separately. It works just fine. The footage above is for testing channel one.
...
### Direct Communication from Node-Red to Arduino
